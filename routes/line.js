...
async function autoRun() {
    // const job = cron.schedule('0 37 * * * *', async function() {
        ...
        //         const qrCodeUrl = await genQr(`${process.env.PATHWEB}?id=${queues[i]._id}`);
        //         const resSendLine = await sendLine(customer[0].tokenLine, queues[i], qrCodeUrl)
...
Router.post("/sendQR", async(req, res, next) => {
    ...
    if (customer && customer.length > 0) {
        if (customer[0].tokenLine) {
            ...
            const resSendLine = await sendLine(customer[0].tokenLine, queue, qrCodeUrl);
            // console.log(resSendLine);
            res.json({ data: "ok" });
        } else {
